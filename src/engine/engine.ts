 ...
 */
export function tickIntervalFor(state: GameState): number {
...
export function pause(state: GameState, now = 0): GameState {
  if (state.status !== 'running') return state;
  return { ...state, status: 'paused', updatedAt: now };
}

export function resume(state: GameState, now = 0): GameState {
  if (state.status !== 'paused') return state;
  return { ...state, status: 'running', updatedAt: now };
}

export function endGame(state: GameState, now = 0): GameState {
 ...
 */
export function tick(state: GameState, rng: Rng = defaultRng, now = 0): GameState {
  if (state.status !== 'running') return state;
